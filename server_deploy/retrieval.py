...
def _load_chunks() -> list[tuple[str, str, np.ndarray]]:
    ...
    if not config.DB_PATH.exists():
        raise RuntimeError(f"Database not found ({config.DB_PATH.name}). Run `python ingest.py` first.")
    ...
    try:
        ...
        conn.close()
    if not rows:
        raise RuntimeError("Database is empty. Run `python ingest.py` first.")
    return [
...
def get_top_chunks(query: str, k: int = config.TOP_K) -> list[dict]:
    ...
    """
    chunks = _load_chunks()
    query_vec = np.array(ollama_client.embed_text(query), dtype=np.float32)

...
def _is_rate_limited(client_ip: str) -> bool:
    """In-memory sliding window: RATE_LIMIT_PER_MINUTE requests per IP per rolling minute."""
    now = time.monotonic()
...
async def vera_qa(request: Request) -> JSONResponse:
    ...
    if _is_rate_limited(get_remote_address(request)):
    ...
    try:
        ...
        answer = ollama_client.complete_chat(messages).choices[0].message.content
    except Exception:
        traceback.print_exc()
        return _error_response(500, "internal_error", FRIENDLY_ERROR)

    declined = "don't have that information" in answer
